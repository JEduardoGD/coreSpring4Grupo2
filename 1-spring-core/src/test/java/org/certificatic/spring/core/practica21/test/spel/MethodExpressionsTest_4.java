...
public class MethodExpressionsTest_4 {
	...
	public void methodExpressionsTest() {
		...
		// definir expresión substring que obtenga el valor esperado de 'Nikola'
		// de 'Nikola Tesla'
		String teslaFirstName = (String) spelParser
				.parseExpression("name.substring(0,6)").getValue(teslaContext);
		...
		// definir expresión que verifique si el 'Rudolf Carl Virchow' es miebro
		// de la sociedad IEEE
		boolean isMemberRudolfCarlVirchow = (boolean) spelParser
				.parseExpression("isMember('Rudolf Carl Virchow')")
		...
		// definir expresión que verifique si el 'Charles Babbage' es miebro de
		// la sociedad IEEE
		boolean isMemberCharlesBabbage = (boolean) spelParser
				.parseExpression("isMember('Charles Babbage')")
				.getValue(societyContext);
		...
		// definir la variable "inventor" con el nombre de "Charles Babbage" y
		// agregarla al contexto sociedad IEEE
		...
		String isMemberCharlesBabbageString = spelParser
				.parseExpression(
						"isMember(#inventor) ? #inventor + ' is member !' : #inventor + ' isn''t member.'")
				.getValue(societyContext, String.class);

		Assert.assertEquals(charlesBabage + " is member !",
				isMemberCharlesBabbageString);

		log.info("isMember CharlesBabbageString: {}",
				isMemberCharlesBabbageString);
