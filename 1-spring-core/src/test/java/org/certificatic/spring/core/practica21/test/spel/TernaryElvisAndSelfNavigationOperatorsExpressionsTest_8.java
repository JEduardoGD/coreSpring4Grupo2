...
public class TernaryElvisAndSelfNavigationOperatorsExpressionsTest_8 {
	...
	public static void setUp() {
		...
		springContext.setBeanResolver(
				applicationContext.getBean(MyBeanResolver.class));
	}
	...
	public void ternaryOperatorExpressionsTest() {
		...
		// define una expresión con operador ternario que devuelva un string u
		// otro dependiendo si el 'nextInt' del objeto random
		// es par o impar.
		String greeting = spelParser
				.parseExpression(
						"#random.nextInt(100) % 2 == 0 ? 'awesome' : 'handsome'")
	...
	public void elvisOperatorExpressionsTest() {
		...
		// define una expresión con operador elvis "?:" que verifique si es nula
		// la
		// variable 'name'
		// si es nula imprimir 'Without me'
		String name = spelParser.parseExpression("#name?:'Without me'")
				.getValue(springContext, String.class);
		...
		name = spelParser.parseExpression("#name?:'Without me'")
				.getValue(springContext, String.class);
		...
		name = spelParser.parseExpression("#name").getValue(springContext,
				String.class); // sólo analiza
		...
		name = spelParser.parseExpression("#name2").getValue(springContext,
				String.class); // sólo analiza
	...
	public void selfNavigationOperatorExpressionsTest2() {
		...
		// define una expresión con operador 'self-navigation' (?.) que obtenga
		// un
		// substring que devuelva el valor 'Ivan'
		String name = spelParser.parseExpression("#name?.substring(0, 4)")
				.getValue(springContext, String.class);
