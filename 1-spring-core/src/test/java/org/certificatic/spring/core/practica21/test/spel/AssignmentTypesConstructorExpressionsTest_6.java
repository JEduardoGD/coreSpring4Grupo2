...
public class AssignmentTypesConstructorExpressionsTest_6 {
	...
	public void assignmentExpressionsTest() {
		...
		String teslaName1 = spelParser.parseExpression("name")
				.getValue(teslaContext, String.class);
		...
		spelParser.parseExpression("name").setValue(teslaContext, "Nikolai"); // sólo
																				// analizar

		String teslaName2 = spelParser.parseExpression("name")
				.getValue(teslaContext, String.class);
		...
		// definir una expresión que asigne y obtenga a la vez el nombre 'Nikola
		// Tesla' sobre el nombre del Inventor Tesla
		String teslaName3 = spelParser.parseExpression("name = 'Nikola Tesla'")
				.getValue(teslaContext, String.class);
	...
	public void typesExpressionsTest() {
		...
		Class<?> dateClass = spelParser.parseExpression("T(java.util.Date)")
				.getValue(Class.class); // sólo analizar

		Assert.assertEquals(Date.class, dateClass);

		// definir una expresión que calcule el área de un circulo de 5.5 de
		// radio (multiplicar y dividir por 1000.0 para truncar decimales
		Double circleArea = spelParser
				.parseExpression(
						"T(Math).round( T(Math).PI * T(Math).pow('5.5', '2') * 10000.0 ) / 10000.0")
		...
		Double doubleNumber = spelParser
				...
				.getValue(Double.class); // sólo analizar
	...
	public void contructorExpressionsTest() {
		...
		GregorianCalendar c = spelParser
				.parseExpression("new java.util.GregorianCalendar()")
		...
		// definir y obtener el valor de una expresión que obtenga el valor
		// (objeto) de un Inventor cuyos datos sean:
		// nombre: 'Albert Einstein', fecha de nacimiento: (la fecha definida en
		// c) y nacionalidad: 'German'
		...
		// definir y obtener el valor de una expresión que obtenga un arreglo de
		// strings cuyos valores sean:
		// 'absorption refrigerator', 'quantum theory', 'theory of general
		// relativity'
		String[] stringArray = spelParser
				.parseExpression("new String[]{ 'absorption refrigerator', "
						+ "'quantum theory', 'theory of general relativity'}")
				.getValue(String[].class);

		inventor.setInventions(stringArray);

		PlaceOfBirth placeOfBirth = spelParser
				.parseExpression(
		...
		Inventor einstein = spelParser
				.parseExpression("getMember('Albert Einstein')")
				.getValue(societyContext, Inventor.class); // sólo analizar
